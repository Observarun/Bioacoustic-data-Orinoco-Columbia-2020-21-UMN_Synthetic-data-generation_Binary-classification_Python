...
The loss function I've used is the sum of binary cross entropy and KL divergence. I used Adam optimiser, which offers a modified form of stochastic gradient descent. I experimented with learning rates of $10^{-3}$, $10^{-4}$, $10^{-5}$, and a stepwise decay w/ learning rate starting with $10^{-4}$ and being multiplied by $0.5$ every $10$ epochs. Since I have only $95$ data points to begin with, I used a small batch size of $32$. For number of epochs, I choose a rather large value of $800$ because of the small dataset.
...
The loss curve (which is a plot of average loss vs epochs) I obtained is quite as expected. The error decreases fast, which is expected from such powerful algorithms, even more so for small datasets. The reconstructed images appear to fall short of the mark, particularly for faint sounds (which are represented by relatively lighter colours). I believe this is on account of very few images in the base dataset. Synthetic images are obviously more distant from the original ones given the latent space random variable is sampled from exactly normal distribution, which is somewhat different from the distribution followed by the from the input data. <!--Using a decaying learning rate isn't making much difference here, possibly because the images contain only low lever features.-->
...
Training DNNs (deep neural networks) typically requires tremendous amount of data and advanced hardware. Here, I'd like to use transfer learning approach to classification, which is relevant in cases where there is insufficient data to train the model from scratch, or if there isn't a GPU available. Transfer learning approach employs models pre-trained on an exising database. A popular example in literature is [ImageNet](https://image-net.org/index.php), which is a dataset of millions of labeled images. A pre-trained model has already learnt its weights on ImageNet dataset. However, these weights still need to be fine-tuned using data relevant to the problem at hand.
