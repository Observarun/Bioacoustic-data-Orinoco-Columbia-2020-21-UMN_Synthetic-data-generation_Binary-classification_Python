...
OpSo offers a way to create a model to classify species using existing machine vision architectures. It abstracts most of the algorithmic and coding details away from the user. (Features that can only be understood from the ccode.) OpenSoundscape initialises the model architectures with weights pre-trained on [Imagenet](https://www.image-net.org/) database by default, which is a dataset of millions of labeled images.
