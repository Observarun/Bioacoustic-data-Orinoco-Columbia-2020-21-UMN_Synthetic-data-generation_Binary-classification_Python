...
Let's call the latent random variable $z$. The encoded distribution $q(z|x)$, where $x$ is the training data. Accordingly, $q(z|x)$ corresponds to the encoder. Decoder is the likelihood $p(x|z)$. Posterior distribution is initially represented by a prior p(x), which is subsequently updated. Let As mentioned earlier, the latent random variable is sampled from the encoded distribution, i.e. $z ~ q(z|x)$. This makes the gradient calculation one of the terms in KL divergence (which is required during backpropagation) cumbersome. To overcome this, the authors proposed a reparametrisation trick, which involves expressing the random variable $z$ as a deterministic variable. For the distribution here, the latent vector can be represented as $z = \mu + \sigma\epsilon$, where $\epsilon$ is an auxillary noise variable $\epsilon \tilde N(0,1)$ (unit normal).
